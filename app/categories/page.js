...
export default function CategoriesPage() {
  ...
  async function handleSaveEdit(e) {
    ...
    try {
      const updated = await updateCategory(editingId, { label: editLabel.trim(), emoji: editEmoji })
      ...
      {loading ? (
        ...
        <div style={{
          ...
          {categories.map((cat, i) => (
            ...
            >
              {editingId === cat.id ? (
                <form onSubmit={handleSaveEdit} style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '12px 16px' }}>
                ...
                >
                  ...
                  <span style={{ flex: 1, fontSize: '15px', fontWeight: 500, color: '#F5EDE8' }}>{cat.label}</span>
                  <button
